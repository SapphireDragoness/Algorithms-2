...
public class AdjMatrixUndirWeight implements WeightedGraph {
	...
	double[][] adjMatrix; // matrice di adiacenza
	private int time = 0;

	/* riempe la matrice con pesi infiniti, tranne gli archi degeneri */
	...
	public boolean containsEdge(String sourceVertex, String targetVertex) throws IllegalArgumentException {
		if (!containsVertex(targetVertex) || !containsVertex(sourceVertex))
			throw new IllegalArgumentException("Vertice inesistente.");
		return (adjMatrix[getVertexIndex(sourceVertex)][getVertexIndex(targetVertex)] != Double.MAX_VALUE && adjMatrix[getVertexIndex(sourceVertex)][getVertexIndex(targetVertex)] != 0)
				&& (adjMatrix[getVertexIndex(targetVertex)][getVertexIndex(sourceVertex)] != Double.MAX_VALUE && adjMatrix[getVertexIndex(sourceVertex)][getVertexIndex(targetVertex)] != 0);
	}
	...
	public void removeEdge(String sourceVertex, String targetVertex)
		...
		setEdgeWeight(sourceVertex, targetVertex, Double.MAX_VALUE);
	}
	...
	@Override
	public WeightedGraph getPrimMST(String startingVertex) throws IllegalArgumentException {
		if (!containsVertex(startingVertex))
			throw new IllegalArgumentException();
		PriorityQueue queue = new PriorityQueue();
		AdjMatrixUndirWeight mst = new AdjMatrixUndirWeight();
		VisitForest visit = new VisitForest(this, null);
		boolean[] definitive = new boolean[vertices.size()];
		
		for(String v : vertices) {	
			visit.setColor(v, Color.WHITE);
			visit.setDistance(v, Double.MAX_VALUE);
			visit.setDistance(startingVertex, 0);
			queue.enqueue(getVertexIndex(v), visit.getDistance(v));
			definitive[getVertexIndex(v)] = false;
		}
		while(!queue.isEmpty()) {
			String u = getVertexLabel(queue.dequeue());
			mst.addVertex(u);
			visit.setColor(u, Color.BLACK);
			definitive[getVertexIndex(u)] = true;
			for(String v : getAdjacent(u)) {
				if(!definitive[getVertexIndex(v)] && visit.getDistance(v) > getEdgeWeight(u, v)) {
					visit.setParent(v, u);
					visit.setDistance(v, getEdgeWeight(u, v));
					queue.modify_priority(getVertexIndex(v), visit.getDistance(v));
				}
			}
		}
		
		/* costruisci MAR a partire dalla visita */
		for(String v : vertices) {
			if(getVertexIndex(v) != 0) {
				String u = visit.getPartent(v);
				mst.addEdge(u, v);
			}
		}
